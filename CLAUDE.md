...
## Current state (keep this true; last updated 2026-09-05 04:14 +0800 (earlier stamp 04:20 was unmeasured))
...
- **Core trade logic** `packages/thetanuts`: round 2 committed `49f7daa` (all 10 round-1 findings folded; 15 tests). Claude leg r2: `.research/thetanuts/review-r2-claude.md` (round trip now matches 4/4 against the SDK, gate present, mutations RED). Sol high reviewer r2 running: `.research/thetanuts/sol-core-review-r2.out`. GREEN from both legs is required before wiring the UI to it.
- **DB** `packages/db`: round 1 committed `db52c88` (schema for PRD §12, migrations 0000+0001, `ThesisAiContext` builder, fixtures, tests). Migrations applied to the LOCAL database and verified. Claude leg: `.research/thetanuts/review-db-r1-claude.md` (no code blocker; gap G1: builder throws for draft/pending theses because PRD §10.2 `contracts` is non-null; needs owner + teammate decision). Sol high reviewer running: `.research/thetanuts/sol-db-review-r1.out`.
- **UI** in worktree `.claude/worktrees/ui-r1` (branch `ui-r1`, base `1b457c4`): Opus worker building the five routes from the mockup; told the zero-confidence rule mid-task. Not reviewed yet.
- Production Supabase: project ref `xentgcambhxdsenfnntj`; `apps/web/.env` holds `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, `PROD_DATABASE_URL` (password URL-encoded). Production schema is applied only by `bunx drizzle-kit migrate` with `DATABASE_URL` set to that URL, after review is GREEN and reachability is verified; never by git push.
- Not started: wallet connect, socials, AI companion (teammate), Vercel.
- Open owner items: every `TODO-OWNER` value listed above, plus G1.
