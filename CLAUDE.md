...
- **Core trade logic** `packages/thetanuts`: round 1 committed as `1b457c4` (sol writer, unreviewed). Claude's review leg is RED: `.research/thetanuts/review-r1-claude.md` (B1 fill re-derives contracts from the floored premium and loses contracts, measured against the SDK; M1 no taker-sell gate in fill; M2 breakEven ignores contract-size scale). Sol high review leg: `.research/thetanuts/sol-core-review-r1.out`. Next: fold both legs into round 2 via the writer, re-review.
