...
## Current state (keep this true; last updated 2026-09-06 01:33 +0800 — a cron rot pass re-measures this section every 30 min)

**Measured now:** main = `bdf5f80` locally AND on GitHub (clean tree). Devfolio project `thesisfun-68be` is PUBLISHED (22:50 on 09-05; live-site link added 23:28; both Thetanuts tracks; six screenshots; details `docs/DEVFOLIO-SUBMISSION.md`). **PRODUCTION DATABASE IS FULLY MIGRATED: measured 01:31 via the session pooler — 9 rows in `drizzle.__drizzle_migrations` (`0000`–`0008`), 14 public tables incl. `users`/`theses`/`positions`, the partial `positions_chain_id_tx_hash_unique` index present.** Someone on the team applied `0001`–`0008` between Claude's 21:16 read (only `0000`) and 01:31; Claude's own `drizzle-kit migrate` run at 01:31 was a no-op (9 → 9). Claude's earlier statement that the live site could not create accounts was WRONG as of 01:29. The team's Vercel deploy `https://nuts-web-kappa.vercel.app/` answers 200 and can now serve sign-in, posts and positions (not yet walked by Claude in the browser).

**Writers RUNNING (all Opus, own worktrees, branches auto-pushed on every commit; each carries the self-distrust block):**
- `fold-C-r3` @ `5a263e3` (worktree `.claude/worktrees/fold-C-r3`; brief `.research/thetanuts/brief-fold-C-r3.md`): the lane C second-pass majors #1–#9 + lane D's D5/D-C1/D-C2 are ALL COMMITTED; the writer is in its verification + headless browser pass + report (`fold-C-r3-report.md`). Killed twice by network outages on 09-05 (22:57, ~23:36), resumed 01:17.
- `fold-A-r2` @ `dccad66` (brief `brief-fold-A-r2.md`): `scripts/sync-vercel-env.ts` must refuse local values / never upload `PROD_*`/`SUPABASE_*` / require `--yes`; `scripts/` joins `check-types` with its 3 diagnostics fixed. Resumed 01:17 after dying at worktree creation.
- `polish-P1` @ `9409d42` (brief `brief-polish-P1.md`): rails print display names per the mockup, the "—" under a rail name, "since Sep 26" wording. Disjoint from fold C's files.
- `polish-P2` @ `7a2ac0e` (brief `brief-polish-P2.md`; defaults authorised by Claude under the owner's common-sense delegation, each `TODO-OWNER`): wallet switch signs the session out (B-m3), OG text strips emoji (no jsDelivr fetch), `ACTIVITY_PAGE_SIZE` on `listActivity`, market page defaults to the first structure with liquidity and a positive max payout.

**Teammate branches (owner 01:33: "my teammate seems to be working on some ui/ux stuff, becareful so no conflicts"):** `origin/agent-ux-r1` (4 own commits: agent replies as markdown, follow-up chips, Agent in the nav, a market-scoped panel + floating launcher, a fomo design digest) is built ON TOP of `fold-C-r3` at `14a05ee` — so it already contains fold C r3's fixes and only `5a263e3` (SDK out of the client bundle) is newer; their own files: `app/api/agent/chat/route.ts`, `app/layout.tsx`, `components/agent/{agent-launcher,agent-markdown}.tsx`, `components/agent/market-link.ts`, `components/shell/nav.tsx`, `index.css`, `styles/agent.css`, `lib/agent/{request,suggestions,tools}.ts`, `package.json`/`bun.lock`. `origin/wallet-ux-r1` (1 commit on `589bd58`: sign-in unlocks the page, visible connect failures) rewrites `components/auth/wallet-bar.tsx` (+ test, `index.css`, new `connect-dialog.tsx`) — overlaps polish-P2's B-m3 change; the P2 writer was told to keep its change to a one-line hook call. Their HANDOVER §12 traced the fold-C-r3 build break (SDK `fs/promises` reaching the client bundle through `display.ts`) — fixed by the writer in `5a263e3`, to be re-measured by a db-mode build. Merge order: my four writers → main; then the teammate rebases/merges their branches (their push rule is not a merge approval); Claude resolves conflicts if asked.
...
**Next, in order:** the four writers report → Claude re-measures each tree (typecheck, offline + live suites on a fresh 0008 throwaway, mutant spot-checks) → merge all → push → the two-leg review (Claude hands-on + Astra MEDIUM, all four lanes at the new pin) → RED findings folded by Opus → repeat until GREEN → (production schema already at 0008) → tell the team to redeploy the merged main → the owner's tiny real fill.
