...
## Current state (keep this true; last updated 2026-09-05 04:10 +08)

- `main` is local-ahead of `origin/main` (last pushed `692e8b2`; origin also had the teammate's PRD commit, merged in). Push is authorized by the owner ONLY once the DB work's two-leg review is GREEN (teammate needs the schema); everything else waits for an explicit push command.
- **Core trade logic** `packages/thetanuts`: round 1 committed as `1b457c4` (sol writer, unreviewed). Claude's review leg is RED: `.research/thetanuts/review-r1-claude.md` (B1 fill re-derives contracts from the floored premium and loses contracts, measured against the SDK; M1 no taker-sell gate in fill; an M2 break-even claim was retracted after measurement). Sol high review leg: `.research/thetanuts/sol-core-review-r1.out`. Next: fold both legs into round 2 via the writer, re-review.
- **UI** in worktree `.claude/worktrees/ui-r1` (branch `ui-r1`, base `1b457c4`): Opus worker building the five routes from the mockup. Not reviewed yet. Harness worktrees are cut from origin/main, so worktrees are created by hand with `git worktree add`.
- **DB** brief ready at `.research/thetanuts/brief-db-schema.md` (PRD §12 tables, migrations, `ThesisAiContext` builder + fixtures); launches when a sol slot frees (max two concurrent sols).
- Production Supabase: project ref `xentgcambhxdsenfnntj`; `apps/web/.env` holds `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, `PROD_DATABASE_URL` (password URL-encoded because it contains `@`). Schema reaches production only by running `bunx drizzle-kit migrate` with `DATABASE_URL` set to that URL, never by git push. Direct host is IPv6 by default; verify reachability before the first migration.
- Not started: wallet connect, socials, AI companion (teammate), Vercel.
- Open owner items: every `TODO-OWNER` value listed above.
