...
## Current state (keep this true; last updated 2026-09-05 04:26 +0800)
...
- **Core trade logic** `packages/thetanuts`: round 2 `49f7daa`. Review r2: Claude leg green on blockers (`.research/thetanuts/review-r2-claude.md`); sol leg RED with 3 majors (quote does not reject zero/expired, receipt ignores emitter, risk allows premium above cap) and 2 minors, all re-measured by Claude. Round 3 fold running from `.research/thetanuts/brief-core-fold-r3.md`. Next: re-measure, reviewer r3.
- **DB** `packages/db`: round 1 `db52c88`; both legs RED (`.research/thetanuts/review-db-r1-claude.md`, `sol-db-review-r1.final.md`): bigint cannot be stored in jsonb snapshots, public-thesis invariant is not fenced in SQL, AI builder trusts unrelated rows, fixtures inconsistent, `numeric(78,0)` rounds. All re-measured by Claude. Round 2 fold running from `.research/thetanuts/brief-db-fold-r2.md` (adds codecs, constraint trigger migration, builder validation, integration tests that Claude runs against the local DB). Next: apply migrations locally, run integration tests, reviewer r2.
- **UI** in worktree `.claude/worktrees/ui-r1` (branch `ui-r1`, base `1b457c4`): Opus worker building the five routes; it screenshots via the browser tool into the shared cwd (`discover.png`, `.playwright-mcp/`, ignored). Not reviewed yet.
- Open owner items: gap G1 (AI context for draft/pending theses needs a PRD §10.2/§15 decision with the teammate), every `TODO-OWNER`.
