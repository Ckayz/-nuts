...
## Current state (keep this true; last updated 2026-09-06 01:29 +0800 — a cron rot pass re-measures this section every 30 min)

**Measured now:** main = `bdf5f80` locally AND on GitHub (clean tree). Devfolio project `thesisfun-68be` is PUBLISHED (22:50 on 09-05; live-site link added 23:28; both Thetanuts tracks; six screenshots; details `docs/DEVFOLIO-SUBMISSION.md`). The team's Vercel deploy `https://nuts-web-kappa.vercel.app/` answers 200 but its database (production Supabase) holds ONLY migration `0000_agent_tables` (six `agent_*` tables; measured 21:16 on 09-05 via the session pooler), so users/theses/positions pages cannot work there until `0001`–`0008` are applied — which happens right after lane C is GREEN (owner rule: migrate green changes without asking).
...
**Next, in order:** the four writers report → Claude re-measures each tree (typecheck, offline + live suites on a fresh 0008 throwaway, mutant spot-checks) → merge all → push → the two-leg review (Claude hands-on + Astra MEDIUM, all four lanes at the new pin) → RED findings folded by Opus → repeat until GREEN → production migration `0001`–`0008` via the session pooler (`DRIZZLE_ALLOW_REMOTE=1 … drizzle-kit migrate`, never `push`) → tell the team to redeploy → the owner's tiny real fill.
