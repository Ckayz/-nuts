...
## Current state (keep this true; last updated 2026-09-06 02:10 +0800 — a cron rot pass re-measures this section every 30 min)

**Measured now (02:10):** main = `0b587f1` locally AND on GitHub (clean tree). It contains: the verified integration `a4394c5` of the four Opus writers (fold-C-r3, fold-A-r2, polish-P1, polish-P2 — Opus verifier `integrate-4-verify.md` GREEN: 0 BLOCKER / 0 MAJOR / 5 MINOR, 14/14 own mutants RED, 8 routes × 2 widths zero console errors; Claude's own measurements matched: check-types clean, web offline 560/57/0, live 700/8/0, db 366/0, thetanuts 51/0, env 15/0, db build) MERGED with the teammate's `2ca44f1` (their agent UI: markdown replies, follow-up chips, Agent in the nav, market-scoped panel + floating launcher; a "From Farcaster" feed rail + truthful empty state; **fomo's palette adopted for `--bg/--surface/--line/--text/--gain/--loss` while keeping OUR accent** — commit `b250adb`; `react-markdown` added). One conflict (`app/t/[slug]/opengraph-image.tsx`) resolved as a union: their palette values + our emoji-safe `ogText` wrappers; `p/[id]/opengraph-image.tsx` auto-merged the same way. After `bun install` (their new package): tsc clean, offline 611/57/0, check-types 5/5; live suites + db build on this tree re-running 02:10. Devfolio PUBLISHED (22:50 on 09-05, live link 23:28). Production database fully migrated (9 rows, 14 tables, measured 01:31). The team's Vercel deploy `https://nuts-web-kappa.vercel.app/` serves main as of their last deploy (not re-walked by Claude).

**Verifier's 5 MINORs (for the review):** MIN-1 if `sessionStorage` is unavailable AND the first `recordTrade` never reaches the server, a remount can send a second fill (`held-fill.ts`'s comment overstates the server fence, which keys on a `pending` row only `recordTrade` inserts); MIN-2 `verify.ts` comment says bun does not load a root `.env` (it does); MIN-3/4/5 `TODO-OWNER` numbers (`UNRECORDED_FILL_WINDOW_MS` 10 min, `ACTIVITY_PAGE_SIZE` 50, the default-structure rule).

**Accent colour (owner 01:3x: "instead of fomo's black and purple. we do black and blueish" → "on the mock up yes go"):** an Opus design worker produced three blue candidates as copies of the mockup under `.research/thetanuts/mockup-blue/` (spec untouched, hash `d49f69d3…`): A `#3b7bff`, B `#5c8ad6`, C `#3f6fe0`; measured white-on-button contrast 3.84 / 3.46 / 4.60 (purple 4.35; the 13.5 px button label needs 4.5, so only C clears it); comparison sheets `ui-fold-shots/accent-compare-{feed,market,dialog}.png` sent to the owner; report `mockup-blue-report.md`. **OWNER PICKED C (02:02: "i guess C is fine?").** An Opus writer (branch `accent-C`, brief `brief-accent-C.md`, launched 02:02) replaces the purple family at its 26 measured occurrences (mockup spec → byte-identical to `blue-C.html`, mockup README, `index.css` tokens + 5 rgba sites, both OG image routes' constants, the CLAUDE.md tokens sentence) and proves zero purple hits + a db-mode build + live shots. The teammate's `wallet-ux-r1`/`agent-ux-r1` touch `index.css` too — token-line-only diff, rebase expected on their side.
