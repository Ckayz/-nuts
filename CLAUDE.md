...
## Current state (keep this true; last updated 2026-09-05 04:36 +0800)
...
- **Core trade logic** `packages/thetanuts`: round 3 `9d82abe`. Claude leg r3 GREEN by probe (`.research/thetanuts/review-r3-claude.md`). Sol high reviewer r3 running (`sol-core-review-r3.out`). Rounds 1–2 history in `review-r1-claude.md`, `sol-core-review-r1.final.md`, `review-r2-claude.md`, `sol-core-review-r2.final.md`.
- **DB** `packages/db`: round 2 `84bc11c` (migrations 0000–0003; snapshot codecs; creator-position constraint triggers; builder validation; integral checks; integration tests). Applied to the LOCAL database and verified live (journal 4, 2 triggers, 16 checks; integration test passes with `bun --env-file=../../apps/web/.env test`). Claude leg r2 GREEN (`review-db-r2-claude.md`). Sol high reviewer r2 running (`sol-db-review-r2.out`). Adds `@thetanuts-finance/thetanuts-client` as a type dependency of `packages/db`.
- **UI** branch `ui-r1` at `4dd56de` in worktree `.claude/worktrees/ui-r1` (base `1b457c4`): five routes on mock data, typecheck and `next build` verified by Claude; screenshot matches the mockup. Not merged; sol review brief ready (`brief-ui-review-template.md`, diff `review-ui-r1.diff`), launches when a sol slot frees. Owner items from the worker: `ThesisStatus` PRD vs mockup naming conflict; `/new` composer has no copy in the mockup; only one thesis has detail data; connected user unnamed; slugs/expiry times tagged `TODO-OWNER`.
