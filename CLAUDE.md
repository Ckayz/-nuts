...
## Current state (keep this true; last updated 2026-09-05 17:30 +0800)

- **main = `9f1ddee` (pushed; `8a76e6c` + this file).** Everything in "Build status" is merged. Verified on main at the last merge (16:2x): `bun run check-types --force` 3/3; thetanuts 50, DB 348 (live, fresh throwaway at 0007), env 10, web 375 live / 275 offline (23 skipped integration cases); `next build` in mock and db mode; a production-build smoke in db mode against a migrated throwaway: every route 200 (unseeded ids 404), zero server errors; browser: `--accent #6f5cff`, Manrope, live structures on `/m/btc`.
- **UI fold round 1 DELIVERED, not yet verified or merged (17:2x):** the Opus HIGH writer left 44 modified + 1 new file UNCOMMITTED in `.claude/worktrees/ui-fold-r1` (branch `ui-fold-r1`, HEAD still `cc555cb`; brief `.research/thetanuts/brief-ui-fold-r1.md`, 25 items; shots in `.research/thetanuts/ui-fold-shots/`). Writer claims (NOT yet re-measured by Claude): offline 277 pass / 23 skip, live 377 pass, check-types 3/3, mock and db builds, db-mode production smoke all 200/404, every route 390 px wide with no overflow; items 3 and 25 refuted with measurements, items 11/22 flagged for the owner (tiles are entry · max loss · max payout, not Risked/Bull/Bear; the mockup hides the trade ticket ≤1180 px). **MAJOR found by the writer and confirmed by Claude at the bytes (17:27):** `apps/web/src/lib/position/instrument.ts:199` maps raw `isLong` to taker `"buy"` — the exact inverse of the chain-verified rule in `packages/thetanuts/src/side.ts:44` (`isLong ? "sell" : "buy"`). It is in main today, not only in the fold. Blast radius: `/p/[id]`, the share card and the fill dialog show "Short the structure (sold)" / "Collateral locked" for a taker-BUY position, and max loss / max payout / break-even derive from the wrong side; `apps/web/src/lib/position/pnl.test.ts` pins the wrong behaviour. Unfixed; ledger item 27; fixed in the one fold after the reviews (or earlier if the owner says so). Lane A review DONE and FOLDED: Astra MEDIUM (pinned worktree @ `510226c`, `review-oneshot/astra-review-A-r2.out`) was RED — MAJOR: the production data-source guard's build-phase exemption let a prerendered mock page be served from cache with no runtime check (Claude's own code; agreed and removed — `next build` now REQUIRES `DATA_SOURCE=db`, measured); MINORs: `PGOPTIONS` bypassed the migrate fence (now refused), DB README stale (fixed). Confirming Astra pass runs with lanes B–D after the UI fold.
- **Review so far (owner rule: usual two-leg review, run once after all code; GREEN before the production migration):** lanes and briefs in `.research/thetanuts/review-oneshot/` (`lane-A-db-env.md`, `lane-B-server-social.md`, `lane-C-trade.md`, `lane-D-ui.md`; `common-header.md` with pin placeholders); the ledger of everything merged unreviewed is `.research/thetanuts/review-ledger.md` (items 1–27). Claude legs done: A (`claude-review-A.md`: no MAJOR; MINORs — `PGOPTIONS`/`PGSERVICE` env bypass the migrate fence; accented letters dropped from slugs), B (`claude-review-B.md`: challenge reuse + single use, draft/self/anon/blank fences, handle race → `handle_taken`, caller-supplied userId ignored, injected display name stored as text — no MAJOR), C partial (`claude-review-C-partial.md`). Lanes B/C/D Astra legs and pins wait for the UI fold to land (their diffs would shift).
- **Next, in order:** verify the fold (re-run its counts, read the diff, browser pass per route vs the mockup shots) → merge (typecheck, suites live, db build, db-mode smoke, browser pass per route vs the mockup shots) → pin lanes B/C/D → Astra medium legs (two at a time, pinned worktrees) + Claude legs → ONE fold of all findings → confirming round → GREEN → production migration (`docs/DEPLOY.md`; needs the owner's session-pooler URL; never `push`) → Vercel (`SESSION_SECRET`, `DATA_SOURCE=db`, pooler `DATABASE_URL`, `OPENROUTER_API_KEY`) → the owner's tiny real fill (`packages/thetanuts/scripts/README.md`).
