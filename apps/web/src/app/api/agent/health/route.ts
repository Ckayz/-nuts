...
import { agentHealth, type ProbeCaller } from "@/lib/agent/health";
 ...
 *   deployment's model credit, so: owner ruling 03:4x, monitors must NOT use it,
 *   and the answer is cached for a window (`PROBE_CACHE_MS`) — a page reload
 *   inside that window re-reads the cached answer instead of calling again.
 *
 * No authentication, by design: it must answer when the agent cannot, and it
 * returns nothing an attacker gains from — a provider name, two model ids that
 * are already in `.env.example`, and a fixed error class. No key, no provider
 * message, no request body, and nothing is written.
 */
