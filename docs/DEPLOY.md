...
Table derived from `packages/env/src/server.ts` and `packages/env/src/web.ts` at `1f0f71f`. Every server key below must also appear in `turbo.json`'s `tasks.build.env`, or a `turbo build` (whose env mode is strict) cannot see it; `packages/env/test/turbo-env.test.ts` asserts that. The web client schema is empty. Requiredness includes the production-only session check. No credentials or configured values belong here; document keys in `apps/web/.env.example`.
...
| AGENT_HEALTH_PROBE_TOKEN | Optional | Operator-generated secret (`openssl rand -hex 16`, at least 16 characters) that `GET /api/agent/health?probe=1` requires before it spends a model call. Absent: the probe is refused and the plain health GET stays config-only |
| RFQ_KEY_MASTER_KEY | Optional; required for RFQ | 32 bytes as 64 hex characters (`openssl rand -hex 32`), generated by the operator. It encrypts every wallet's RFQ key at rest in `agent_rfq_keys`. Absent: every RFQ preparation is refused with one sentence and no key is ever written in clear -- the correct safe state, and a deliberate go/no-go. **Never rotate it without a re-encryption pass: rotation orphans every stored key permanently, and an RFQ created under a lost key can never have its offers decrypted** |
