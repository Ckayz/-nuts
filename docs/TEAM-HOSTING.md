...
| `AGENT_MODEL`, `AGENT_GATE_MODEL`, `BASE_RPC_URL`, `THETANUTS_ORDERS_URL`, `THESIS_REFERRER` | as in `apps/web/.env.example` | Optional with defaults. `AGENT_MODEL` defaults to `anthropic/claude-haiku-4.5` (a `:free` OpenRouter id is NOT served by Vercel AI Gateway and fails every turn when `AI_GATEWAY_API_KEY` is set); `AGENT_GATE_MODEL` stays on the paid `anthropic/claude-haiku-4.5`. `THESIS_REFERRER` defaults to the owner's referrer wallet — keep it, that is the revenue address. |
