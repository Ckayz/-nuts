...
export async function seed(client: Client, ids = { u1, u2, t1, t2, p1 }, unique = false) {
  ...
  await client.query("INSERT INTO public.theses(id,slug,creator_user_id,headline,tagged_asset,direction,status,underlying_asset,expiry_at,product_type,is_call,is_long,strikes,strike_decimals,collateral_address,collateral_symbol,collateral_decimals,creator_order_snapshot) VALUES ($1,'h-' || replace(($1::uuid)::text,'-',''),$2,'h','ETH','bull','draft','ETH',now(),'call',true,true,ARRAY[1]::numeric[],8,'0xc','USDC',6,'{}'),($3,'h2-' || replace(($3::uuid)::text,'-',''),$2,'h2','ETH','bull','draft','ETH',now(),'call',true,true,ARRAY[1]::numeric[],8,'0xc','USDC',6,'{}')", [t1, u1, t2]);
